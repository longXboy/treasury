...
import (
	...
	"github.com/longxboy/treasure/ent/migrate"
...
type Client struct {
	...
	// Confirm is the client for interacting with the Confirm builders.
...
func (c *Client) init() {
	...
	c.Confirm = NewConfirmClient(c.config)
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.Confirm.Intercept(interceptors...)
...
// ConfirmClient is a client for the Confirm schema.
type ConfirmClient struct {
	config
}

// NewConfirmClient returns a client for the Confirm from the given config.
func NewConfirmClient(c config) *ConfirmClient {
	return &ConfirmClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `confirm.Hooks(f(g(h())))`.
func (c *ConfirmClient) Use(hooks ...Hook) {
	c.hooks.Confirm = append(c.hooks.Confirm, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `confirm.Intercept(f(g(h())))`.
func (c *ConfirmClient) Intercept(interceptors ...Interceptor) {
	c.inters.Confirm = append(c.inters.Confirm, interceptors...)
}

// Create returns a builder for creating a Confirm entity.
func (c *ConfirmClient) Create() *ConfirmCreate {
	mutation := newConfirmMutation(c.config, OpCreate)
	return &ConfirmCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Confirm entities.
func (c *ConfirmClient) CreateBulk(builders ...*ConfirmCreate) *ConfirmCreateBulk {
	return &ConfirmCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ConfirmClient) MapCreateBulk(slice any, setFunc func(*ConfirmCreate, int)) *ConfirmCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ConfirmCreateBulk{err: fmt.Errorf("calling to ConfirmClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ConfirmCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
// Update returns an update builder for Confirm.
func (c *ConfirmClient) Update() *ConfirmUpdate {
	mutation := newConfirmMutation(c.config, OpUpdate)
	return &ConfirmUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *ConfirmClient) UpdateOne(co *Confirm) *ConfirmUpdateOne {
	mutation := newConfirmMutation(c.config, OpUpdateOne, withConfirm(co))
	return &ConfirmUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ConfirmClient) UpdateOneID(id int64) *ConfirmUpdateOne {
	mutation := newConfirmMutation(c.config, OpUpdateOne, withConfirmID(id))
	return &ConfirmUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Confirm.
func (c *ConfirmClient) Delete() *ConfirmDelete {
	mutation := newConfirmMutation(c.config, OpDelete)
	return &ConfirmDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *ConfirmClient) DeleteOne(co *Confirm) *ConfirmDeleteOne {
	return c.DeleteOneID(co.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ConfirmClient) DeleteOneID(id int64) *ConfirmDeleteOne {
	builder := c.Delete().Where(confirm.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &ConfirmDeleteOne{builder}
}

// Query returns a query builder for Confirm.
func (c *ConfirmClient) Query() *ConfirmQuery {
	return &ConfirmQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeConfirm},
		inters: c.Interceptors(),
	}
}

// Get returns a Confirm entity by its id.
func (c *ConfirmClient) Get(ctx context.Context, id int64) (*Confirm, error) {
	return c.Query().Where(confirm.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ConfirmClient) GetX(ctx context.Context, id int64) *Confirm {
...
func (c *ConfirmClient) Hooks() []Hook {
	return c.hooks.Confirm
}

// Interceptors returns the client interceptors.
func (c *ConfirmClient) Interceptors() []Interceptor {
	return c.inters.Confirm
}

func (c *ConfirmClient) mutate(ctx context.Context, m *ConfirmMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ConfirmCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ConfirmUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ConfirmUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ConfirmDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Confirm mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Confirm, Request []ent.Hook
	}
	inters struct {
		Confirm, Request []ent.Interceptor
	}
)
